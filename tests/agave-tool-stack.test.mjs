...
test("tool logos expand with a four-pixel gap and subtle hover elevation", () => {
  assert.match(styles, /width: 20px;[\s\S]*?var\(--tool-index\) \* 18px/);
  assert.match(styles, /\.engCardTitleRow\s*\{[^}]*gap: 6px/);
  assert.match(source, /zIndex: index \+ 1/);
  assert.match(styles, /\.engCard:hover \.engCardTool\s*\{[^}]*transform: translateX\(calc\(4px \+ var\(--tool-index\) \* 24px\)\)/);
  ...
  assert.match(styles, /@media \(hover: none\)[\s\S]*?2px \+ var\(--tool-index\) \* 24px/);
  assert.match(styles, /@media \(prefers-reduced-motion: reduce\)[\s\S]*?\.engCardTool\s*\{[^}]*transition: none/);
});

test("project tool logos default to twenty pixels with Cursor at twenty-one", () => {
  assert.match(styles, /\.engCardTool\s*\{[^}]*width: 20px;[^}]*height: 20px/);
  assert.match(styles, /\.engCardTool:has\(img\[src\$="cursor-2\.png"\]\)\s*\{[^}]*width: 21px;[^}]*height: 21px/);
